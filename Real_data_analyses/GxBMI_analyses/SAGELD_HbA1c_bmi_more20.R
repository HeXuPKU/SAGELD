...
# library(GRAB)
# 
# longpheno = data.table::fread("your_working_directory/HbA1c.txt")
# 
...
# 
# longpheno = longpheno %>%
#   drop_na() %>% filter(bmi > 20) %>%
#   filter(feid %in% SubjIDinPLINK$V1) %>%
#   filter(feid %in% IDs_have_genotypes) %>%
#   select(feid, a1c, age, sex_genetic, medication, PC1:PC10, bmi) %>%
#   mutate(a1c = scale(a1c), age = scale(age), age2 = scale(age^2), bmi = scale(bmi)) %>%
#   mutate(age_sex = age * sex_genetic, age_med = age * medication)
# 
# # nullmodel = lmer(a1c ~ sex_genetic + PC1 + PC2 + PC3 + PC4 + PC5 + PC6 + PC7 + PC8 + PC9 + PC10 + medication + age + age2 + age_sex + age_med + (age | feid),
# #                  data = longpheno, control = lmerControl(optimizer ='optimx', optCtrl = list(method = 'nlminb')))
# 
# nullmodel = glmmTMB::glmmTMB(a1c ~ sex_genetic + PC1 + PC2 + PC3 + PC4 + PC5 + PC6 + PC7 + PC8 + PC9 + PC10 + medication + age + age2 + age_sex + age_med + bmi + (bmi | feid), data = longpheno)
# 
...
#      file = "your_working_directory/HbA1c_bmi.RData")
...
precond.SAGELD = "your_working_directory/HbA1c_bmi.RData"
load(precond.SAGELD)
