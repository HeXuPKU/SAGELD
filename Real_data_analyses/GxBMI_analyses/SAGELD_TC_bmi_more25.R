...
# library(GRAB)
# 
# longpheno = data.table::fread("your_working_directory/TC.txt")
# 
...
#   mutate(tc = scale(tc), age = scale(age), age2 = scale(age^2), bmi = scale(bmi)) %>%
#   mutate(age_sex = age * sex_genetic, age_med = age * medication)
# 
# nullmodel = lmer(tc ~ sex_genetic + PC1 + PC2 + PC3 + PC4 + PC5 + PC6 + PC7 + PC8 + PC9 + PC10 + medication + age + age2 + age_sex + age_med + bmi + (bmi | feid),
...
#      file = "your_working_directory/TC_bmi.RData")
...
precond.SAGELD = "your_working_directory/TC_bmi.RData"
load(precond.SAGELD)
