...
# library(GRAB)
# 
# longpheno = data.table::fread("your_working_directory/LDL.txt")
# 
...
# 
# longpheno = longpheno %>%
#   drop_na() %>% filter(bmi > 25) %>%
#   filter(feid %in% SubjIDinPLINK$V1) %>%
#   filter(feid %in% IDs_have_genotypes) %>%
#   mutate(ldl = scale(ldl), age = scale(age), age2 = scale(age^2), bmi = scale(bmi)) %>%
#   mutate(age_sex = age * sex_genetic, age_med = age * medication)
# 
# nullmodel = lmer(ldl ~ sex_genetic + PC1 + PC2 + PC3 + PC4 + PC5 + PC6 + PC7 + PC8 + PC9 + PC10 + medication + age + age2 + age_sex + age_med + bmi + (bmi | feid),
...
#      file = "your_working_directory/LDL_bmi.RData")
...
precond.SAGELD = "your_working_directory/LDL_bmi.RData"
load(precond.SAGELD)
