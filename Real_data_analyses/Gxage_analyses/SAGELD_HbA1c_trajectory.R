...
#   select(feid, a1c, age, sex_genetic, medication, PC1:PC10) %>%
#   mutate(a1c = scale(a1c), age = scale(age), age2 = scale(age^2)) %>%
...
# 
# nullmodel = glmmTMB::glmmTMB(a1c ~ sex_genetic + PC1 + PC2 + PC3 + PC4 + PC5 + PC6 + PC7 + PC8 + PC9 + PC10 + medication + age + age2 + age_sex + age_med + (age | feid), data = longpheno)
# 
...
#      file = "your_working_directory/HbA1c_trajectory.RData")
...
precond.SAGELD = "your_working_directory/HbA1c_trajectory.RData"
load(precond.SAGELD)
