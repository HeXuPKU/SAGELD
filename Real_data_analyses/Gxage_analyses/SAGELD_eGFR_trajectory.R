...
# library(GRAB)
# 
# longpheno = data.table::fread("your_working_directory/eGFR.txt")
# 
...
#   mutate(egfr = scale(egfr), age = scale(age), age2 = scale(age^2)) %>%
#   mutate(age_sex = age * sex_genetic)
# 
# nullmodel = lmer(egfr ~ sex_genetic + PC1 + PC2 + PC3 + PC4 + PC5 + PC6 + PC7 + PC8 + PC9 + PC10 + age + age2 + age_sex + (age | feid),
...
#      file = "your_working_directory/eGFR_trajectory.RData")
...
precond.SAGELD = "your_working_directory/eGFR_trajectory.RData"
load(precond.SAGELD)
