...
# library(GRAB)
# 
# longpheno = data.table::fread("your_working_directory/TG.txt")
# 
...
#   mutate(tri = scale(tri), age = scale(age), age2 = scale(age^2)) %>%
#   mutate(age_sex = age * sex_genetic, age_med = age * medication)
# 
# nullmodel = lmer(tri ~ sex_genetic + PC1 + PC2 + PC3 + PC4 + PC5 + PC6 + PC7 + PC8 + PC9 + PC10 + medication + age + age2 + age_sex + age_med + (age | feid),
...
#      file = "your_working_directory/TG_trajectory.RData")
...
precond.SAGELD = "your_working_directory/TG_trajectory.RData"
load(precond.SAGELD)
