...
longpheno = pheno_simu(nSub = nSub, nFam = nFam, FamMode = "10-members", 
                       medianObs = medianObs, Env = "quan", betaG = 1, betaGxE = 0.5,  
...
reduce_pheno1 = longpheno %>% group_by(SubjID) %>% arrange(SubjID) %>%
  ...
  select(FID, IID, slope, xone, xtwo, xthree) %>% drop_na %>% ungroup %>% 
  filter(slope > quantile(slope)[2] - 3 * (quantile(slope)[4] - quantile(slope)[2])) %>%
  filter(slope < quantile(slope)[4] + 3 * (quantile(slope)[4] - quantile(slope)[2]))
...
GEMcommoand = paste0("/gdata01/user/xuhe/software/GEM_1.5.3_Intel ",
                     "--bfile your_working_director/causalSNPs ",
                     "--pheno-file your_working_director/scenario", scr, "/", Envtype, medianObs, "/Crosssectionalun.", nrep, ".txt ",
                     "--sampleid-name FID --pheno-name pheno --exposure-names Env --covar-names xone xtwo xthree --robust 1 --threads 1 ", 
                     "--out your_working_director/scenario", scr, "/", Envtype, medianObs, "/Crosssectional.", nrep, ".GEM.txt")
