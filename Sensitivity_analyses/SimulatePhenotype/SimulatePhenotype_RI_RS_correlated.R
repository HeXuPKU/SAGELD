...
pheno_simu = function(nSub,                           # number of independent individuals.
                      ...
                      randomemain = NULL,             # random effect for environment main effect.
                      randomGxE = NULL,               # random effect for interaction effect.
                      ...
                      rho = 0.8)                      # weight parameter to weight random intercept and slope effects.
{
  ...
  if(is.null(randomemain)){
    randomemain = rep(0, N)
  }else if(length(randomemain) != N){
    stop("Length of random vector should correspond to number of individuals.")
  }
  if(is.null(randomGxE)){
    randomGxE = rep(0, N)
  }else if(length(randomGxE) != N){
    stop("Length of random vector should correspond to number of individuals.")
  }
  ...
  randomeffect_main = sqrt(rho) * rep(randomemain, times = m);
  
  randomeffect_GxE = sqrt(1 - rho) * rep(randomGxE, times = m) * Env;
