...
pheno_simu = function(nSub,                           # number of independent individuals.
                      ...
                      Env = "quan",                   # time invariant quantitative/binary variable.
                      ...
                      rho = 0.8)                      # weight parameter to weight random intercept and slope effects.
{
  ...
  if(Env == "quan")
  {
    Env = rnorm(N, 0, 1); Env = rep(Env, times = m)    # time in-variant standard normal variable.
  }else if(Env == "binary")
  {
    Env = rbinom(N, 1, 0.5); Env = rep(Env, times = m) # time in-variant binary variable.
  }else
  {
