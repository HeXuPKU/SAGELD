...
pheno_simu = function(nSub,                           # number of independent individuals.
                      ...
                      Skewness = "Normal",            # whether Continuous environmental variables are skewed (Normal, Moderate, High).
                      ...
                      rho = 0.8)                      # weight parameter to weight random intercept and slope effects.
{
  ...
  if(Env == "quan")
  {
    ...
    if(Skewness == "Normal")
    {
      # Baseline
      Env = rnorm(M, 0, 1)
    }else if(Skewness == "Moderate")
    {
      # Modern skewed
      Env = rgamma(M, shape = 1, scale = 1)
      Env = scale(Env)
    }else if(Skewness == "High")
    {
      Env = rgamma(M, shape = 0.1, scale = 1)
      Env = scale(Env)
    }else
    {
      stop("Only support Normal, Modern, High")
    }
