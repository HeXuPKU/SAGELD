...
polygenicGxE = GenoMatSD %*% rnorm(causalnum, sd = sqrt(1/causalnum)) %>% as.numeric()

nSub = 25000; nFam = 2500
longpheno = pheno_simu(nSub = nSub, nFam = nFam, FamMode = "10-members", 
                       medianObs = 4, Env = "binary", MedFreq = 0.7, MedPrpo = 0.3, betaG = 1, betaGxE = 1,  
                       Geno = NULL, bVectomain = polygenicmain, bVectoGxE = polygenicGxE, rho = 0.8)
...
GRAB.Marker(objNull = SAGELD_NULL_Model,
            ...
            OutputFile = paste0("your_working_directory/", Envtype, causalnum, "/SAGELD.", nrep, ".txt"),
